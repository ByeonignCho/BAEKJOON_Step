...
# 노이즈 제거
    ...
    # 추가 로직 -점선 gap 보정 - 0813-15
    # ===== 점선 gap 보정 =====
    vertical_sum = np.sum(binary_img, axis=1)
    gap_threshold = 5  # 이 값 이하의 빈 부분은 채움
    for i in range(1, len(vertical_sum)-1):
        if vertical_sum[i] == 0 and vertical_sum[i-1] > 0 and vertical_sum[i+1] > 0:
            binary_img[i, :] = 255  # gap 채우기
    # 0813-15 end
...
def sliding_window(binary_img, image):
    ...
    global pre_lx, pre_rx, pre_left_x, pre_right_x
    global flag, pre_target_x, pre_steer, last_lane_detect_time
    nwindows = 15
    margin = 30
    minpix = 30
...
# 이전 sliding window된 값에서 너무 크게 벗어날 경우 
    ...
    if len(lx) >= 3 and len(rx) >= 3:
        ...
        lx = [int(np.polyval(left_fit, y)) for y in ly]
        rx = [int(np.polyval(right_fit, y)) for y in ry]

    # 시각화
    for i in range(len(lx)):
        image = cv2.circle(image, (lx[i], ly[i]), 5, (0,0,255), -1)
        image = cv2.circle(image, (rx[i], ry[i]), 5, (0,0,255), -1)
    # 0813-15 end
    ...
    # ✅ 중심점 보정 로직 (차선 사라짐 대응)
    # ====================================
    lane_width = pre_right_x - pre_left_x
    lane_width = np.clip(lane_width, 380, 480)

    if cnt_l == 0 and cnt_r != 0:
        left_x = right_x - lane_width
        print("왼쪽 차선 없음 → 오른쪽 기준 추론")

    elif cnt_r == 0 and cnt_l != 0:
        right_x = left_x + lane_width
        print("오른쪽 차선 없음 → 왼쪽 기준 추론")

    elif cnt_r == 0 and cnt_l == 0:
        left_x = pre_left_x
        right_x = pre_right_x
        print("양쪽 차선 없음 → 이전 값 사용")

    target_x = (left_x + right_x) / 2

    # ====================================
    # ✅ target_x 스무딩 처리 (관성 보정)
    # ====================================
    alpha = 0.8
    target_x = alpha * pre_target_x + (1 - alpha) * target_x
    ...
    if cnt_l > 0 or cnt_r > 0:
        last_lane_detect_time = rospy.Time.now()
